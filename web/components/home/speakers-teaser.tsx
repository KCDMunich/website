  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-primary/8 to-primary/[0.03] ring-1 ring-primary/10">
      ...
      <div className="space-y-3 p-5">
        <div className="mx-auto h-5 w-3/4 animate-pulse rounded bg-primary/10" />
        <div className="mx-auto h-4 w-1/2 animate-pulse rounded bg-primary/10" />
...
export function SpeakersTeaser() {
  ...
  const featuredSpeakers = getFeaturedSpeakers(speakerData);

  return (
    <Section id="speakers" className="bg-background">
        ...
        <MotionReveal>
          ...
          <p className="mt-5 text-lg leading-relaxed text-muted-foreground">
            {isLoading || speakerData.length === 0
              ? "Practitioners and experts from the cloud native community — more speakers announced soon."
              : `${Math.min(FEATURED_SPEAKER_COUNT, speakerData.length)} highlights from ${speakerData.length} speakers in the lineup.`}
          </p>
        </MotionReveal>
      </div>
      ...
      <div className="mt-12 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3 lg:gap-5">
        {isLoading
          ? Array.from({ length: FEATURED_SPEAKER_COUNT }).map((_, index) => (
              <SpeakerCardSkeleton key={index} />
            ))
          : featuredSpeakers.map((speaker, index) => (
              <MotionReveal key={speaker.id} delay={index * 0.06}>
                <SpeakerCard
                  speaker={speaker}
