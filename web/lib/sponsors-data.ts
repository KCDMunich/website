...
export const SPONSOR_CONTACT_EMAIL = EVENT_CONFIG.community.sponsorEmail;

export const SPONSOR_PROSPECTUS_URL = EVENT_CONFIG.community.sponsorProspectusUrl;
...
const LOGO_BASE: Record<SponsorTier, { height: number; maxWidth: number }> = {
...
export const sponsorsByEdition: Partial<Record<number, readonly Sponsor[]>> = {
  2026: [
    {
      name: 'APE Factory',
...
export function getSponsorsForEdition(edition: number): readonly Sponsor[] {
  const sponsors = sponsorsByEdition[edition];
  if (!sponsors) {
    throw new Error(`Missing sponsor configuration for edition ${edition}.`);
  }
